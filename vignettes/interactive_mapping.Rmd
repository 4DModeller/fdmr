...
Colour palettes can be selected based on the type of data being plottied. Qualitative, sequential or diverging colour
palettes can be selected using the interface. If you want more control over the colour palette you use you can pass in a colour
palette to the `palette` argument. Below is an example of how we create the default colour palette within `plot_interactive_map`.

WIP

```{r}
# domain = raster::values(raster_data[[layer_name]])
# pal <- colorNumeric(
#             palette = "Blues",
