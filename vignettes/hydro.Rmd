...
We want to mark the position of the dam and the stream gauges on the map so we'll create a `data.frame` to pass into the `plot_map_leaflet`
...
```{r}
fdmr::plot_map_leaflet(polygon_data = sfc, markers = markers, polygon_fill_opacity = 0.5)
```
