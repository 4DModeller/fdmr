...
```{r}
markers <- data.frame(latitude = c(59.490720, 59.5815849, 59.7531662),
                    longitude = c(6.517174, 6.5395789, 6.7897968),
                    label = c("Suldalsvatnet Dam", "Stream Gauge 13", "Stream Gauge"))
```
...
We can plot the average stream flow using `plot_timeseries`
```{r}
...
We rescale the data with the zscore since the values are so different between the two stream gauge s
