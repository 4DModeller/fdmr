...
```{r}
poly_path <- fdmr::get_tutorial_datapath(filename = "Kvilldal_Catch_Boundary.geojson")
...
norway_polygon <- rgdal::readOGR(poly_path)
...
```{r}
fdmr::plot_map(polygon_data=sfc, markers=markers, fill_opacity = 0.5)
```
...
s13_utm <- fdmr::latlong_to_utm(lat=stream_gauge_13$latitude
                          ,lon=stream_gauge_13$longitude)
s14_utm <- fdmr::latlong_to_utm(lat=stream_gauge_14$latitude
...
```{r}
for (i in seq_along(nrow(pixel_coords))) {
...
Now we create a new `SpatialPointDataFrame` called `inla_data` for model fitting. The data frame contains the response and predictor data at each spatial location and time point.
...
# Maybe this makes the mesh smaller
...
inlabru.model1 <- inlabru::bru(formula1, data = inla_data,
                   family = "gaussian",
                   options = list(
                     verbose = TRUE
                   )
...
inlabru.model2 <- inlabru::bru(formula2, data = inla_data,
                   family = "gaussian",
                   options = list(
                     verbose = TRUE
                   )
