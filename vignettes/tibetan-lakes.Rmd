...
data <- read.csv('~/repos/tblakes/data_for_4dm_w_temp.csv')
...
# formula <- zscore_water_balance_m3 ~ 0 + Intercept(1)  +  zscore_precip  +  f(
#                 main = coordinates,
#                 model = spde,
#                 group = group_index,
#                 ngroup = n_groups,
...
# formula2 <- zscore_water_balance_m3 ~ 0 + Intercept(1) + precipitation(data_with_zscores$zscore_precip, model='linear')
formula2 <- zscore_water_balance_m3 ~ 0 + Intercept(1) + precipitation(data_with_zscores[, c('centroid_lon', 'centroid_lat')], model='spde', group=group_index, ngroup=n_groups, control.group=list(model='ar1', hyper=alphaprior))
