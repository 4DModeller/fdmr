...
```
```{r}
sp::coordinates(raindata) <- c("Longitude", "Latitude")
```
...
```{r}
run_output <- fdmr::fit_model(y="Rain", data=raindata, process_coords = raindata, time_variable="Time_Index", latitude_col = "Latitude", longitude_col = "Longitude")
```
