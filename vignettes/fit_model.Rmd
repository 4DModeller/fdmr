...
```{r}
fit_model <- function(location_data = NULL, exposure_parameter, spatial_data, mesh = NULL, formula = NULL, mesh_params = NULL, function_params = NULL, model_params = NULL) {
  if (is.null(mesh)) {
    mesh <- fdmr::create_mesh(location_data, spatial_data)
  }
  if (is.null(formula)) {
    formula <- fdmr::create_formula(exposure_parameter, mesh)
  }
  fdmr::model_fit(formula, data = location_data, ...)
}
```
