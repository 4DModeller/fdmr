...
```{r eval=FALSE}
tarpath <- "/example/path/covid19_example.tar.bz2"
fdmr::retrieve_tutorial_data(tarball_path = tarpath)
```

Next we'll use the `load_tutorial_data` function to load in the spatial data we want.

```{r loadshape, error=TRUE, eval=FALSE}
sp_data <- fdmr::load_tutorial_data(filename = "spatial_data.rds")
```

Now we make a map of the study region

```{r rmap, error=TRUE,fig.cap="A map of the study region.",fig.width=4,fig.height=4,fig.align='center', eval=FALSE}
sp_data@data$mapp <- 0
domain <- sp_data@data$mapp

fdmr::plot_map(data = sp_data, domain = domain, add_scale_bar = TRUE, polygon_fill_opacity = 0.5, fill_colour_weight = 0.6)
```
...
```{r loadCOVIDat, error=TRUE, eval=FALSE}
...
```{r plotcases, error=TRUE, fig.cap="The weekly reported number of COVID cases between March 7, 2020 and March 26, 2022.",fig.width=6,fig.height=4, fig.align = "center", eval=FALSE}
...
```{r createmesh, error=TRUE, eval=FALSE}
...
```{r plotmesh, error=TRUE, fig.cap="Triangulated mesh used to build the SPDE model.",fig.width=5,fig.height=5, fig.align='center', eval=FALSE}
point_data <- sp_data@data[, c("LONG", "LAT")]

<<<<<<< HEAD
```{r error=TRUE, plotmesh, fig.cap="Triangulated mesh used to build the SPDE model.",fig.width=5,fig.height=5, fig.align='center'}
plot(mesh,main='')
points(sp.dat@data[,c("LONG","LAT")],col="blue",cex=0.1)
print(sp.dat)
=======
...
```{r formula, error=TRUE, eval=FALSE}
...
# inlabru.model<-bru(formula, data = COVID19dat,
#                    family = "poisson",
#                    E = COVID19dat$Population,
...
```{r fixedeffects, error=TRUE, eval=FALSE}
...
```{r hyperpar, error=TRUE, eval=FALSE}
...
```{r posteriordis,error=TRUE,fig.cap="Posterior distributions of the spatial range, standard deviation and temporal dependence parameters.",fig.width=8,fig.height=2, fig.align = "center", eval=FALSE }
list_marginals <- list(
...
pred.mean <-
  inlabru_model$summary.fitted.values$mean[1:nrow(covid19_data)]
pred.25 <-
  inlabru_model$summary.fitted.values$`0.025quant`[1:nrow(covid19_data)]
pred.975 <-
  inlabru_model$summary.fitted.values$`0.975quant`[1:nrow(covid19_data)]
...
```{r loadpreds, error=TRUE, eval=FALSE}
...
```{r boxps, fig.cap="Boxplots of the predicted infection rates across all neighbourhoods over time",fig.width=6,fig.height=4, fig.align = "center", eval=FALSE}
breaks_vec <- c(seq(as.Date("2020-03-07"),
...
```{r plotmeanrisk,error=TRUE,fig.cap="The average predicted infection risk in weeks",fig.width=6,fig.height=4, fig.align = "center", eval=FALSE}
...
```{r mapaverisk, error=TRUE, fig.cap="Map of the average predicted infection risks ",fig.width=6,fig.height=4, fig.align = "center", eval=FALSE}
average_risk_by_nb <-
...
```{r performance, error=TRUE, eval=FALSE}
...
```{r performancePlots,error=TRUE,fig.cap="RMSE, bias and coverage probability for the model over time.",fig.width=8,fig.height=2, fig.align = "center", eval=FALSE}
