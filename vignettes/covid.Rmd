...
fdmr::plot_map(polygon_data = sp_data, domain = domain, add_scale_bar = TRUE, polygon_fill_opacity = 0.5, fill_colour_weight = 0.6)
```

```{r}
colours <- colorNumeric(palette = "YlOrRd", domain = sp_data@data$mapp, reverse=FALSE)
leaflet(data=sp_data) %>%addTiles() %>%
  addPolygons(fillColor = ~colours(mapp), color="grey", weight=0.6,
              fillOpacity = 0.5) %>%
...
average_risk_by_nb<- group_by(predictions,MSOA11CD) %>% summarize(
  ave.risk = mean(pred.mean))

sp_data@data$ave.risk<-average_risk_by_nb$ave.risk

colours <- colorNumeric(palette = "Reds", domain = sp_data@data$ave.risk, reverse=FALSE)
leaflet(data=sp_data) %>% 
  addTiles() %>% 
  addPolygons(fillColor = ~colours(sp_data@data$ave.risk),
              color="transparent", weight=1, fillOpacity = 0.8) %>%
  addLegend(pal = colours,values = sp_data@data$ave.risk,opacity = 0.8,
                       title="risk")
```
