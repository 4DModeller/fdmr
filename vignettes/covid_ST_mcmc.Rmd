...
W.nb <- spdep::poly2nb(sp_data,row.names = rownames(sp_data@data))
...
In order to fit the model, a model formula needs to be defined, by including the response in the left-hand side and the fixed and random effects in the right-hand side. We select a few risk factors used in the COVID-19 tutorial [https://4dmodeller.github.io/fdmr/articles/covid.html](https://4dmodeller.github.io/fdmr/articles/covid.html). 
...
covars<-unique(st_covid[,c("MSOA11CD",
                  "IMD",
                  "age1",
                  "perc.chinese",
                  "perc.indian",
...
for (i in 1:nrow(dat)){         #  this will take a few minutes
  if(is.na(dat$Population[i])){
    ...
    dat$Population[i]<-nbhoods[match(dat[i,"MSOA11CD"],
                                     nbhoods$MSOA11CD),
                               "Population"]
...
MCMC_model <-ST.CARar(formula=form, 
                      data=st_covid, 
                      family="poisson", 
                      W=w,
                      burnin=10000, 
...
modfit<-data.frame(DIC=c(modfits[1],inla_sum[1]),
...
regr_est<-cbind.data.frame("MCMC"=mod_sum[1:6,1],
...
mcmc.mean.rate<- group_by(mergedat,MSOA11CD) %>% summarize(
  mean.rate = mean(mcmc.fitted.prev))
sp_data@data$mcmc.mean.rate<-mcmc.mean.rate$mean.rate
domain <- sp_data@data$mcmc.mean.rate
fdmr::plot_map(
...
INLA.mean.rate<- group_by(mergedat,MSOA11CD) %>% summarize(
  mean.rate = mean(inla.fitted.prev))
sp_data@data$INLA.mean.rate<-INLA.mean.rate$mean.rate
domain <- sp_data@data$INLA.mean.rate
fdmr::plot_map(
  polygon_data = sp_data,
  domain = domain,
  palette = "Reds",
