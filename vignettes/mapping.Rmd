...
norway_polygon_filepath <- fdmr::get_tutorial_datapath(dataset = "hydro", filename = "Kvilldal_Catch_Boundary.geojson")
```

Next we read in the shapefile and transform to the coordinate reference system (CRS) we need to plot it on the `leaflet` map.

```{r}
norway_polygon <- sf::st_zm(sf::read_sf(norway_polygon_filepath))
...
```{r}
norway_polygon_filepath <- fdmr::get_tutorial_datapath(dataset = "hydro", filename = "Kvilldal_Catch_Boundary.geojson")
norway_polygon <- sf::read_sf(norway_polygon_filepath) %>% sf::st_zm()
