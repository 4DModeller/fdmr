...
> **_NOTE:_**  The above will be converted to use the `fdmr::plot_map` function

Maybe have to get rid of domain here if it isn't need

```{r eval=FALSE}
plot_map(data=sfc, tiles="satellite")
```
...
```{r}
fdmr::plot_map(data=sfc, raster_image=raster_image, add_polygons=TRUE)
```
